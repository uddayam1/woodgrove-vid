...
public class PresentController : ControllerBase
{
    ...
    [AllowAnonymous]
    [HttpGet("/api/SelfIssued/Present")]
    ...
    public async Task<ResponseToClient> Get()
    {
        ...
        Status status = new Status("SelfIssued", "Present");
        ...
        try
        {
            ...
            // The constraints contains a collection of claims constraints that must be met when a wallet selects a credential. 
            // In this case we request a credential that the consent claim value is set to 'yes'.
